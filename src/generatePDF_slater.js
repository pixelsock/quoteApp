...
import { default as autoTable } from 'https://cdn.skypack.dev/jspdf-autotable';
...
function generatePDF(quoteItems, finalTotal, quoteNumber, sku) {
  ...
  // Add the logo
  const logoUrl =
    'https://uploads-ssl.webflow.com/638fbc9b6d164e234dc677d7/64e27f2a344c637a5c2038d4_logo.png'; // Replace with the path or base64 string of your logo
  ...
  pdf.setFontSize(16);
  pdf.text("Sales Quote", pdf.internal.pageSize.width - 40, 50, null, null,
    'right'); // Adjust position as needed
  pdf.setFontSize(12);
  const quoteDate = new Date();
  pdf.text(`Quote Date: ${quoteDate.toLocaleDateString()}`, pdf.internal.pageSize.width - 40, 80,
    null, null, 'right'); // Adjust position as needed
  const expiryDate = new Date(quoteDate);
  expiryDate.setDate(expiryDate.getDate() + 90);
  pdf.text(`Expiry Date: ${expiryDate.toLocaleDateString()}`, pdf.internal.pageSize.width - 40, 100,
    null, null, 'right'); // Adjust position as needed
  pdf.text("Customer Number: 12345", pdf.internal.pageSize.width - 40, 120, null, null,
    'right'); // Adjust position as needed
