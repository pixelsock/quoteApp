...
function addFilteredItemToQuote() {
    ...
    const sizeString = currentSelections['size'].match(/\d+/g).join('');
    const mirrorSize = parseInt(sizeString, 10);
    ...
    let freightCharge = newCalculateFreightCharge(mirrorSize, quantity);
    console.log('mirrorSize:', mirrorSize);
...
function newCalculateFreightCharge(mirrorSize, quantity) {
    // Adjusting calculation to correctly handle mirror sizes and quantities
    let baseCharge = 0;
    if (mirrorSize === 1836 || mirrorSize === 2436) { // Mirrors smaller than 36"x36"
        baseCharge = 50;
    } else if ([2448, 3042, 3048, 3232, 3636, 3642].includes(mirrorSize)) { // Mirrors larger than 36"x36"
        baseCharge = 275;
    } else {
        // For sizes not explicitly listed, defer to a total quantity-based calculation
        return calculateFreightChargeForTotal(quantity);
    }
    // Apply base charge up to a quantity of 6, otherwise use total quantity-based calculation
    return quantity <= 6 ? baseCharge * quantity : calculateFreightChargeForTotal(quantity);
}

function calculateFreightChargeForTotal(totalQuantity) {
    // Adjusted tiered pricing based on total quantity
    if (totalQuantity <= 6) {
        return 350; // Corrected to include quantity of 6 in the first tier
